...
export const checkBrickConnections = bricks => {
  const connectedBricks = new Set()
  bricks.forEach(brick => {
    bricks.forEach(otherBrick => {
      if (brick.uuid === otherBrick.uuid) return
      const intersects = brick.completionBoundingBoxIntersects(otherBrick.completionBoundingBox)
      if (intersects) connectedBricks.add(brick.uuid, otherBrick.uuid)
    })
  })
  return connectedBricks.size === bricks.length
}
