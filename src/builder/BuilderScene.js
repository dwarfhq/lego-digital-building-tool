...
import { map, mapConstrain, mainBuilderColor, playClickSound } from '../js/utils'
...
export class BuilderScene extends Scene {
  ...
  makeGround() {
    ...
    const gridHelper = new GridHelper(size, divisions, mainBuilderColor, mainBuilderColor)
  ...
  makeDragControl() {
    ...
    this.$pointer.on('start', ({ x, y }) => {
      ...
      this.intersectedObjects = this.raycaster.intersectObjects(this.bricks.meshes())
      ...
      if (this.intersectedObjects.length > 0) {
        ...
        this.brickPosition.set(mesh.position.x, mesh.position.y, mesh.position.z)

        this.collidableBricks = this.bricks
          .collection()
          .filter(({ uuid }) => !this.aboveBricks.find(item => item.uuid === uuid))
          .filter(({ uuid }) => {
    ...
    this.$pointer.on('move', ({ x, y, xv, yv }) => {
      ...
      const distanceToCameraLookAt = this.$camera.position.distanceTo(cameraLookAtCenter)

      const aWorldToLocalMap =
        mapConstrain(y, 0, 1, 2, map(this.$camera.rotation.y, 0, 1.5, 1, 5)) * 0.25
      const { offsetWidth, offsetHeight } = this.$renderer.domElement
      const bWorldToLocalMap = mapConstrain(offsetWidth / offsetHeight, 0.5, 1.5, 0.25, 0.75)
    ...
    this.$pointer.on('end', ({ x, y }) => {
      ...
      if (!this.bricks.brickNotRotateable(mesh.$)) {
        ...
        if (duration < 200 && duration > 50) {
          ...
          mesh.rotation.y = parseInt((mesh.rotation.y + rotate + 0.01) / rotate) * rotate
  ...
  render() {
    ...
    if (!this.isDragging) {
      ...
      if (brick) {
        ...
        this.brickPosition.x += Math.round(direction.x / BrickFactory.unit) * BrickFactory.unit
        this.brickPosition.z += Math.round(direction.z / BrickFactory.unit) * BrickFactory.unit
      }
    }
  }
}
